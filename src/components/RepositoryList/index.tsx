...
const categorizePR = (title: string) => {
  const categories = [
    { keys: ['FEAT', 'NOVO', 'ADICIONA', 'REP'], label: 'Nova Funcionalidade' },
...
const getProblemType = (categoryLabel: string) => {
  switch (categoryLabel) {
    case 'Nova Funcionalidade': return 'new';
    case 'Correção de Bug': return 'error';
    case 'Correção Urgente': return 'warning';
    case 'Documentação': return 'info';
    case 'Build/Dependências': return 'warning';
    case 'Melhoria de Performance': return 'warning';
    case 'Estilo/Formatação': return 'info';
    case 'Refatoração': return 'info';
    case 'Tarefas Administrativas': return 'info';
    case 'Integração Contínua': return 'info';
    case 'Configurações/Dados': return 'info';
    case 'Testes': return 'info';
    case 'Em Andamento': return 'warning';
    case 'Reversão': return 'warning';
    case 'Aperfeiçoamento': return 'info';
    case 'Segurança': return 'error';
    case 'Infraestrutura/Deploy': return 'warning';
  ...
  return (
    <Stack borderRadius={2} padding={2} flex={1}>
      {groupedPullRequests.length === 0 && loading && (
        <Typography color="text.primary" textAlign="center" mt={4}>
          Carregando PRs dos repositórios...
        </Typography>
      )}

      {groupedPullRequests.map((group) => (
        <Stack key={group.repo} mb={4}>
          <Typography fontSize={'1.2rem'} fontWeight={600} my={2}>
            {group.repo}
          </Typography>
          <Stack
            width="100%"
            ...
            flexWrap="wrap"
          >
            {group.prs.length === 0 && loading ? (
              Array.from({ length: 2 }).map((_, idx) => (
                <CardItemSkeleton key={`skeleton-${group.repo}-${idx}`} />
              ))
            ) : (
              group.prs.map((prs) => {
                const category = categorizePR(prs.title);
                const isLoading = prs.isLoading || loadingPrIds.includes(prs.id);
                  ...
                  >
                    <a href={prs.prUrl} target="_blank" rel="noreferrer">
                      <CardItem
                        ...
                        isLoading={isLoading} // <- flag que ativa skeleton interno
                      />
                    </a>
                  </Box>
                );
